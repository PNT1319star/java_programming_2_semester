    ...
    public int executeCommand(String[] commandSet) {
        ...
        try {
            if (commandType.equals(CommandType.SIMPLE)) {
                Request request = new Request(commandSet[0]);
                processCommand(request);
                return 1;
            } else if (commandType.equals(CommandType.OBJECT)) {
                Request request = new Request(commandSet[0], OrganizationCreator.organizationCreator(scanner));
                processCommand(request);
                return 1;
            } else if (commandType.equals(CommandType.ARGUMENT)) {
                Request request = new Request(commandSet[0], commandSet[1]);
                processCommand(request);
                return 1;
            } else if (commandType.equals(CommandType.UPDATE)) {
                Request request = new Request(commandSet[0], OrganizationCreator.organizationCreator(scanner), commandSet[1]);
                processCommand(request);
                return 1;
